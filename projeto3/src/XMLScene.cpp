...
XMLScene::XMLScene(char *filename)
{
	...
	parseDisplayLists(this->sceneGraph->getRootNode());
...
void XMLScene::parseNodesTransformation(){
	...
	for(int i=0; i<sceneGraph->getGraphNodes().size(); i++){
		Node *n = sceneGraph->getGraphNodes()[i];
		nodeTransforms = n->getTransforms();
		glLoadIdentity();
		for(int j=0; j<nodeTransforms.size(); j++){
			nodeTransforms[j]->apply();
		}
...
void XMLScene::parseDisplayLists(Node *root){
	vector<Node *> nodeDescendants = root->getDescendants();

	glMultMatrixf(root->getTransformationMatrix());

	for (int i = 0; i < nodeDescendants.size(); i++){
		glPushMatrix();
		parseDisplayLists(nodeDescendants[i]);
		glPopMatrix();
	}

	if (root->getHasDisplayList()){

		if (root->isCalculated() == false) {
			int index = glGenLists(1);
			root->setIDDisplayList(index);
			
			glNewList(root->getIDDisplayList(), GL_COMPILE);
...
void XMLScene::visitNode(Node *root, bool applyLocalTransformation) {
	...
	if (root->getHasDisplayList() && root->isCalculated()){
		glCallList(root->getIDDisplayList());
	}
	else {
		if (root->getPrimitives().size() != NULL) {
			root->getAppearance()->apply();
			for (int i = 0; i < root->getPrimitives().size(); i++){
				root->getPrimitives()[i]->draw();
			}
		}
		...
		for (int i = 0; i < nodeDescendants.size(); i++){
			glPushMatrix();
			visitNode(nodeDescendants[i], true);
			glPopMatrix();
		}
	}
}
...
/*
		...
		//Calling child DisplayLists
		for (int i = 0; i < nodeDescendants.size(); i++){
			if (nodeDescendants[i]->getHasDisplayList()) {
				glCallList(nodeDescendants[i]->getIDDisplayList());
			}
			else {
				parseDisplayLists(nodeDescendants[i]);
			}
...
void XMLScene::parseTextures(TiXmlElement *texturesElement){
	...
	for (TiXmlElement *texture = texturesElement->FirstChildElement("texture"); texture != NULL; texture = texture->NextSiblingElement("texture")) {
		...
		if(texture->Attribute("texlength_t") == NULL)
			std::cout << "Could not load textleng_t attribute" << std::endl;
...
std::vector<Primitives *> XMLScene::parsePrimitives(TiXmlElement *primitivesElement){
	 ...
	 for (TiXmlElement *primitive = primitivesElement->FirstChildElement(); primitive != NULL; primitive = primitive->NextSiblingElement()) {
		...
		//TRIANGLE
			...
			if(primitive->Attribute("xyz2")!=NULL && sscanf(primitive->Attribute("xyz2"),"%f %f %f",&x2,&y2,&z2) == 3){
			}else{
				std::cout << "Could not load primitive attributes" << std::endl;
			}

			if(primitive->Attribute("xyz3")!=NULL && sscanf(primitive->Attribute("xyz3"),"%f %f %f",&x3,&y3,&z3) == 3){
			}else{
				std::cout << "Could not load primitive attributes" << std::endl;
			}
		...
		//PLANE
			...
			if (primitive->Attribute("parts") == NULL)
				std::cout << "Could not load primitive attributes" << std::endl;
			else{
				parts = (int)std::stof(primitive->Attribute("parts"));
			}
		...
		} else if((!strcmp(primitive->Value(), "board"))){
			prim = new Board(drawstyle,normalstyle);
		}
...
std::vector<Transforms *> XMLScene::parseTransforms(TiXmlElement *transformsElement){
	...
	for (TiXmlElement *transform = transformsElement->FirstChildElement("transform"); transform != NULL; transform = transform->NextSiblingElement("transform")) {
		...
		//SCALE
			...
			if (transform->Attribute("factor") != NULL && sscanf(transform->Attribute("factor"), "%f %f %f", &tox, &toy, &toz) == 3){
			}else{
				std::cout << "Could not load transform attributes" << std::endl;
			}
			float factor[3];
...
Node * XMLScene::parseNodes(TiXmlElement *nodesElement){
	...
	if(nodesElement->Attribute("displaylist")){
		if(!strcmp(nodesElement->Attribute("displaylist"), "true"))
			n1->setDisplayList(true);
		else if (!strcmp(nodesElement->Attribute("displaylist"), "false"))
			n1->setDisplayList(false);
		else
			std::cout << "Invalid displaylist value." << std::endl;
	}
	...
	for (TiXmlElement *node = nodesElement->FirstChildElement(); node != NULL; node = node->NextSiblingElement()){
		 if (!strcmp(node->Value(), "transforms")){
			 std::vector<Transforms *> transf = parseTransforms(node);

            for (int i=0; i<transf.size(); i++)
                n1->addTransform(transf[i]);
			 ...
			 std::vector<Primitives *> prims = parsePrimitives(node);

			 for (int i=0; i<prims.size(); i++)
				 n1->addPrimitive(prims[i]);
